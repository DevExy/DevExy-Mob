  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await prefs.setString('user', jsonEncode({'username': _usernameController.text}));
        setState(() => _showSuccess = true);
      } else {
        setState(() => _error = data['detail'] ?? 'Invalid username or password');
      }
    } catch (e) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          if (_showSuccess) SuccessPopup(message: 'Login successful', onClose: _handlePopupClose),
        ],
      ),
    );
  }
